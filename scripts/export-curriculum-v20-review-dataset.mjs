      ...
      for (const semester of track.semesters) {
        for (let unitIndex = 0; unitIndex < semester.units.length; unitIndex += 1) {
          ...
          rows.push({
            ...
            v18Validation: inspected?.validation ?? null,
            content: inspected?.unit ?? null,
          })
        }
      }
    }
...
await writeFile('v20-review-export.json', JSON.stringify({ version: 'v20-review-export-2026-08-18', rows }, null, 2), 'utf8')
console.log(`[v20-export] wrote ${rows.length} full unit records to v20-review-export.json`)
