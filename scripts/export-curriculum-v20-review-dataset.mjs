...
try {
  ...
  const finalReview = await server.ssrLoadModule('/src/curriculum-textbook-v20-reviewed-final.ts')
    ...
    for (const route of plan.getCurriculumRouteOptions(grade)) {
      ...
      for (const semester of track.semesters) {
        for (let unitIndex = 0; unitIndex < semester.units.length; unitIndex += 1) {
          ...
          const content = finalReview.getTextbookUnitContentV20ReviewedFinal(unit.id)
...
await writeFile('v20-review-export.json', JSON.stringify({ version: 'v20-reviewed-final-export-2026-08-18', rows }, null, 2), 'utf8')
console.log(`[v20-export] wrote ${rows.length} final reviewed V20 learner unit records to v20-review-export.json`)
