...
const CH_CLASSIC = /文言|古典文本初探|古典散文|古文與思想|文化經典|經典整合閱讀/
const CH_POETRY = /詩歌|詩詞|古典詩|新詩/
const CH_ARGUMENT = /論說|論證|公共議題|學術與公共論述|論述與資訊/
const CH_WRITING = /寫作|作文|圖文寫話|日記與書信|口語表達|專題表達|國中素養統整|語文素養統整/
...
function expected(subject, pathway, title) {
  if (subject === 'math') {
    if (title === '數量關係') return /第 1 項|起始量|增加次數|每次增加量/
    if (title === '整數運算與科學記號') return /氣溫|科學記號|10\^|小數點/
    if (title === '國中數學統整') return /x²|因式|角|機率|中位數/
    if (title === '二次函數') return /拋物線|頂點|頂點式|\(x-.*\)²/
    if (title === '實數與代數') return /絕對值|到 0 的距離|\|-/
    ...
    if (/多項式|因式分解|平方根|二次方程式/.test(title)) return /x²|展開|因式|畢氏|平方根/
  ...
  if (subject === 'science') {
    if (title === '科學觀察與測量') return /測量|0 刻度|刻度|單位/
    if (title === '植物的身體') return /植物|根|水分|呼吸|吸水/
    if (title === '動物與環境') return /蹼|蹄|棲地|環境|構造/
    if (title === '水與物質') return /鹽|溶解|水溫|攪拌|水量/
    if (title === '力與磁力') return /磁鐵|磁力|含鐵|吸引|塑膠/
    if (title === '月亮與天空') return /月相|月球|受光|公轉|亮面/
    if (title === '材料與變化') return /融化|燃燒|新物質|可逆|化學變化/
    if (title === '電磁現象') return /電流|磁場|指南針|線圈/
    if (title === '微小世界與生物') return /顯微鏡|細胞|洋蔥|微小/
    if (title === '波、聲音與光') return /頻率|音調|反射角|入射角|反射定律/
    if (title === '天文與宇宙') return /地球|月球|太陽|公轉|衛星/
    if (pathway === 'physics' && title === '電場與電位') return /電場|電位能|正功|保守電力/
    if (pathway === 'chemistry' && title === '環境與能源化學') return /生命週期|塑膠|材料|排放|資源|廢棄/
    ...
    if (pathway === 'physics') {
      ...
      if (/電|磁/.test(title)) return /電壓|電流|電場|電位|W|磁通|楞次/
    ...
    if (/水循環|天氣|地球|太空|地質/.test(title)) return /蒸發|凝結|月相|自轉|板塊|氣候/
  ...
  if (subject === 'social') {
    if (title === '地方與國家變遷') return /老照片|地方志|史料|過去|變遷/
    ...
    if (/歷史|過去|史前|政權|殖民|戰後|文明|革命|工業化|冷戰|時代|近現代|變遷/.test(title)) return /史料|老照片|訪談|時間線|歷史/
    ...
    for (const route of plan.getCurriculumRouteOptions(grade)) {
      ...
      for (const sem of track.semesters) {
        for (const unit of sem.units) {
          ...
          if (!pattern) {
            failures.push(`${unit.id} ${unit.title}: no semantic family classification`)
            continue
          }
          classified += 1
          const content = final.getTextbookUnitContentV20ReviewedFinal(unit.id)
          if (!content) {
            failures.push(`${unit.id} ${unit.title}: missing final content`)
            continue
          }
          const taskText = norm(`${(content.questions ?? []).map((q) => `${q.context} ${q.prompt} ${q.kind === 'choice' ? (q.options ?? []).join(' ') : q.sampleAnswer} ${q.explanation}`).join(' ')} ${(content.workedExamples ?? []).map((e) => `${e.context} ${e.prompt} ${e.answer} ${e.explanation}`).join(' ')}`)
            .replaceAll(unit.title, '')
            .replaceAll(unit.focus, '')
...
if (classified !== 453 || failures.length) {
  console.error('[curriculum-v20-semantic-alignment] FAILED')
