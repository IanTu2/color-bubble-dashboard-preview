...
function uniqueChoices(answer: string, candidates: string[]) {
  const filtered = Array.from(new Set(candidates.filter((item) => item && item.toLowerCase() !== answer.toLowerCase())))
  return shuffle([answer, ...shuffle(filtered).slice(0, 3)])
}
...
function profileLevel(profile: LearnerProfile): GeneratedCefrLevel {
  return LEVELS.includes(profile.level as GeneratedCefrLevel) ? profile.level as GeneratedCefrLevel : 'A2'
}

function nearbyLevel(level: GeneratedCefrLevel, offset: number) {
...
function chooseEntry(
  ...
  const available = entries.filter((entry) => !recent.includes(`${entry.id}:${entry.level}`))
  const pool = available.length > 20 ? available : entries
  const at = (target: GeneratedCefrLevel) => pool.filter((entry) => entry.level === target)
  ...
  if (range === 'learned') {
    ...
    return learned.length > 0 ? pick(learned) : safePick(at(level))
  }
  if (range === 'challenge') {
    return Math.random() < 0.75 ? safePick(at(nearbyLevel(level, 1))) : safePick(at(level))
  }

  const difficult = new Set(history.difficultWordIds)
  const difficultPool = pool.filter((entry) => difficult.has(entry.id) && [level, nearbyLevel(level, -1)].includes(entry.level))
  if (stats.lastScore < 0.5 && Math.random() < 0.55) return safePick(at(nearbyLevel(level, -1)))
  if (stats.streak >= 5 && Math.random() < 0.28) return safePick(at(nearbyLevel(level, 1)))

  const roll = Math.random()
  if (roll < 0.35 && difficultPool.length > 0) return pick(difficultPool)
  if (roll < 0.75) return safePick(at(level))
  if (roll < 0.9) return safePick(at(nearbyLevel(level, -1)))
  return safePick(at(nearbyLevel(level, 1)))
}
...
function buildChallenge(entry: GameEntry, previous: ChallengeMode | null, language: Language): Challenge {
  ...
  const mode = pick(modes.filter((item) => item !== previous).length > 0 ? modes.filter((item) => item !== previous) : modes)
  ...
  if (mode === 'unscramble') {
    return {
      ...
      entry,
      mode,
      prompt: language === 'zh' ? `重新排列字母：${letters(entry.word)}` : `Unscramble: ${letters(entry.word)}`,
      answer: entry.word,
      acceptedAnswers: [],
      context: `${entry.level} · ${entry.pos}`,
  ...
  if (mode === 'repair') {
    return {
      ...
      hint: language === 'zh' ? `可用字母：${letters(entry.word)}` : `Letters: ${letters(entry.word)}`,
    }
  }
...
export function EnglishCasualPracticeV4({ language, profile, history, setHistory, onOpenWord }: Props) {
  const entries = useMemo(buildEntries, [])
  const level = profileLevel(profile)
  const initialStats: Stats = { attempts: 0, points: 0, streak: 0, bestStreak: 0, lastScore: 1 }
  const [range, setRange] = useState<GameRange>('adaptive')
  const [recent, setRecent] = useState<string[]>([])
  const [stats, setStats] = useState<Stats>(initialStats)
  const [challenge, setChallenge] = useState<Challenge>(() => buildChallenge(chooseEntry(entries, 'adaptive', level, history, [], initialStats), null, language))
  ...
  const grade = (answerText: string, unknown = false) => {
    ...
    const sourceNote = challenge.entry.detail?.memory ?? (language === 'zh'
      ? `此題來自 ${challenge.entry.source}；大量詞庫只提供可驗證的分級、詞性、發音與拼字資料。`
      : `This item comes from ${challenge.entry.source}; the large bank uses verifiable level, POS, audio, and spelling data.`)

    setResponse(answerText)
    setFeedback({ score: result.score, text: `${smartGradeLabel(result, challenge.answer, language)} ${sourceNote}` })
    setStats((current) => {
  ...
  const moveNext = (nextRange = range, reset = false) => {
    ...
    const nextStats = reset ? initialStats : stats
    ...
    setResponse('')
    setFeedback(null)
    if (reset) setStats(initialStats)
  }
