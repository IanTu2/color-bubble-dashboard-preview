...
import type { AppPreferences, AnimationLevel } from '../preferences'
...
export function SettingsDialog({
  ...
  const copy = language === 'zh'
    ? {
        close: '關閉',
        title: '設定',
        subtitle: '調整顯示、背景動畫、音樂與工作區行為。',
        language: '語言',
        appearance: '顯示',
        fontSize: '字體大小',
        motion: '背景動畫',
        motionHint: '降低動畫可減少電腦負擔，也比較不容易分心。',
        low: '柔和',
        normal: '標準',
        high: '活潑',
        bubbles: '泡泡數量',
        backgroundTools: '背景工具',
        music: '常駐音樂播放器',
        musicHint: '啟用後，右下角會顯示小型播放器，可展開完整音樂工作室。',
        workspace: '工作區',
        rememberWindows: '記住開啟中的視窗',
        rememberHint: '重新整理後恢復記事本與搜尋視窗；關閉的視窗不會恢復。',
        saved: '設定會保存在這個瀏覽器中。',
        reset: '恢復所有預設設定',
      }
    : {
        close: 'Close',
        title: 'Settings',
        subtitle: 'Adjust display, background motion, music, and workspace behavior.',
        language: 'Language',
        appearance: 'Display',
        fontSize: 'Font size',
        motion: 'Background motion',
        motionHint: 'Lower motion reduces system load and visual distraction.',
        low: 'Gentle',
        normal: 'Standard',
        high: 'Lively',
        bubbles: 'Bubble count',
        backgroundTools: 'Background tools',
        music: 'Persistent music player',
        musicHint: 'Shows a compact player at the bottom-right with an expandable music studio.',
        workspace: 'Workspace',
        rememberWindows: 'Remember open windows',
        rememberHint: 'Restores note and search windows after refreshing. Closed windows stay closed.',
        saved: 'Settings are saved in this browser.',
  ...
  const animationOptions: Array<{ value: AnimationLevel; label: string }> = [
    ...
    { value: 'normal', label: copy.normal },
  ...
  return (
    <div className="modal" role="presentation" onMouseDown={onClose}>
      <section
        className="settings-shell settings-shell-wide"
        role="dialog"
        aria-modal="true"
        ...
        <div className="settings-grid">
          ...
          <section className="settings-section settings-section-span">
            <div className="settings-row settings-row-start">
              <div>
                <h3>{copy.motion}</h3>
                <p className="setting-description">{copy.motionHint}</p>
              </div>
            </div>
            <div className="segmented settings-segmented motion-options">
              {animationOptions.map((option) => (
                <button
                  className={`language-option${preferences.animationLevel === option.value ? ' active' : ''}`}
                  type="button"
                  key={option.value}
                  onClick={() => updatePreferences({ animationLevel: option.value })}
                >
                  {option.label}
                </button>
              ))}
            ...
            <div className="settings-row settings-slider-row">
              <label htmlFor="bubble-count">{copy.bubbles}</label>
              <output>{preferences.bubbleCount}</output>
            </div>
            <input
              id="bubble-count"
              className="font-size-range"
              ...
              max="36"
              step="6"
              value={preferences.bubbleCount}
              onChange={(event: ChangeEvent<HTMLInputElement>) => updatePreferences({ bubbleCount: Number(event.target.value) })}
            />
          </section>
          ...
          <section className="settings-section">
            ...
            <div className="settings-toggle-row">
              <div>
                <h3>{copy.music}</h3>
                <p className="setting-description">{copy.musicHint}</p>
              </div>
              <button
                className={`settings-switch${preferences.musicEnabled ? ' active' : ''}`}
                type="button"
                role="switch"
          ...
          <section className="settings-section">
            ...
            <div className="settings-toggle-row">
              <div>
                <h3>{copy.rememberWindows}</h3>
                <p className="setting-description">{copy.rememberHint}</p>
              </div>
              <button
                className={`settings-switch${preferences.rememberWindows ? ' active' : ''}`}
                type="button"
                role="switch"
                aria-checked={preferences.rememberWindows}
                onClick={() => updatePreferences({ rememberWindows: !preferences.rememberWindows })}
              >
                <span />
              </button>
            </div>
