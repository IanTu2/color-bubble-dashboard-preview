...
function selectAssessmentQuestion(
  ...
  const ranked = candidates
    .map((item) => {
      const abilityDistance = Math.abs(item.difficulty - ability)
      const skillBalancePenalty = counts[item.skill] * 0.32
      const recentPenalty = recentIds.includes(item.id) ? 2.8 : 0
      const randomVariation = Math.random() * 0.72
      return {
        item,
        score: abilityDistance + skillBalancePenalty + recentPenalty + randomVariation,
      }
    })
    ...
    return (
      <div className="english-studio english-welcome">
        <div className="english-hero">
          ...
          <h2>{copy.title}</h2>
          <p>{copy.subtitle}</p>
          <div><span>Adaptive</span><span>Typing</span><span>Listening</span><span>CEFR</span><span>Random Bank</span></div>
        </div>
          ...
          <section className="english-card">
            ...
            <div className="english-chip-grid">
              {goalOptions.map((goal) => (
                <button
                  className={profile.goals.includes(goal.id) ? 'active' : ''}
                  type="button"
                  key={goal.id}
    ...
    return (
      <div className="english-studio english-assessment">
        <header className="assessment-head">
          <div>
            <p className="eyebrow">ADAPTIVE PLACEMENT · RANDOMIZED</p>
            <h2>{copy.question} {answers.length + 1} / {ASSESSMENT_LENGTH}</h2>
          </div>
          <div className="assessment-level">{englishLevelFromNumber(ability)}</div>
        </header>

        <div className="assessment-progress">
          <span style={{ width: `${Math.min(100, (answers.length / ASSESSMENT_LENGTH) * 100)}%` }} />
        </div>

        <section className="assessment-card">
          <div className="assessment-meta">
            <span>{skillLabel(question.skill, language)}</span>
            <span>Difficulty {question.difficulty.toFixed(1)}</span>
          </div>
          <h3>{question.prompt}</h3>
          {question.context ? <p>{question.context}</p> : null}
          {question.type === 'listening' ? (
            <button className="listen-button" type="button" onClick={() => speakEnglish(question.audioText ?? question.answer, profile.accent)}>🔊 {copy.listen}</button>
          ) : null}
          ...
          <form onSubmit={submitAssessment}>
            ...
            {!feedback ? (
              ...
              <div className={`assessment-feedback score-${feedback.score}`}>
                ...
                <button className="primary-button" type="button" onClick={nextAssessmentQuestion}>
                  {answers.length >= ASSESSMENT_LENGTH ? copy.result : copy.next}
                </button>
              </div>
            )}
          </form>
    ...
    return (
      <div className="english-studio english-result">
        ...
        <div className="result-level-card">
          ...
          <div>
            <strong>{result.confidence === 'high' ? 'High confidence' : 'Medium confidence'}</strong>
            <p>{copy.averageTime}：{result.averageSeconds}s</p>
          </div>
        </div>
        <div className="skill-result-grid">
          {(Object.keys(result.skillScores) as EnglishSkill[]).map((skill) => (
            <article key={skill}>
              <div><strong>{skillLabel(skill, language)}</strong><span>{result.skillScores[skill]}%</span></div>
              <div><span style={{ width: `${result.skillScores[skill]}%` }} /></div>
            </article>
          ))}
        </div>
          ...
          <section className="practice-card">
            <div className="practice-head">
              <div>
                ...
                <h3>{practiceWord.word}</h3>
                <span>{practiceWord.phoneticUS} · {practiceWord.partOfSpeech}</span>
              </div>
            ...
            {practiceFeedback ? (
              <div className="practice-feedback">
                ...
                <button type="button" onClick={advancePractice}>{practiceStep === PRACTICE_MODES.length - 1 ? copy.finishWord : copy.nextStep}</button>
              </div>
            ) : null}
        ...
        <div className="english-word-library">
          ...
          <div className="word-card-grid">
            {filteredWords.length === 0 ? <p>{copy.noWords}</p> : filteredWords.map((word) => (
              <button
                className={history.learnedWordIds.includes(word.id) ? 'learned' : history.difficultWordIds.includes(word.id) ? 'difficult' : ''}
                type="button"
                key={word.id}
            ...
            <section className="progress-skill-panel">
              {(Object.keys(result.skillScores) as EnglishSkill[]).map((skill) => (
                <div key={skill}>
                  ...
                  <div><i style={{ width: `${result.skillScores[skill]}%` }} /></div>
                  <strong>{result.skillScores[skill]}%</strong>
                </div>
              ))}
            </section>
          ) : null}
      ...
      {tab === 'settings' ? (
        <div className="english-settings-panel">
          <section>
            <h3>{copy.goals}</h3>
            <div className="english-chip-grid">
              {goalOptions.map((goal) => <button className={profile.goals.includes(goal.id) ? 'active' : ''} type="button" key={goal.id} onClick={() => toggleGoal(goal.id)}>{goal.label}</button>)}
            </div>
          </section>
          <section>
            <h3>{copy.minutes}</h3>
            <div className="english-setting-line">
              <input type="range" min="5" max="60" step="5" value={profile.dailyMinutes} onChange={(event) => setProfile((current) => ({ ...current, dailyMinutes: Number(event.target.value) }))} />
              <strong>{profile.dailyMinutes} {copy.minuteUnit}</strong>
            </div>
          </section>
          <section>
            <h3>{copy.accent}</h3>
          ...
          <p>{copy.save}</p>
          <div className="english-danger-actions">
            <button type="button" onClick={beginAssessment}>{copy.retest}</button>
            <button
              type="button"
              onClick={() => {
      ...
      {selectedWord ? (
        <div className="word-detail-backdrop" role="presentation" onMouseDown={() => setSelectedWordId(null)}>
          <article className="word-detail" onMouseDown={(event) => event.stopPropagation()}>
            <header>
              <div>
                <p className="eyebrow">WORD MEMORY MAP</p>
                <h3>{selectedWord.word}</h3>
                <span>{selectedWord.phoneticUS} / {selectedWord.phoneticUK}</span>
              </div>
              <button type="button" aria-label={copy.closeDetail} onClick={() => setSelectedWordId(null)}>×</button>
            </header>
            <div className="word-detail-meaning">
              <strong>{selectedWord.meaning}</strong>
              <span>{selectedWord.partOfSpeech} · Level {selectedWord.level}</span>
