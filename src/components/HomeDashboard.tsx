        ...
        <aside className="member-dashboard" aria-label={copy.dashboard}>
          ...
          >
            <div className="card-head">
              <div><p className="eyebrow">{copy.calendar}</p><h2>{monthLabel}</h2></div>
              <span className="card-view-label">{copy.view} →</span>
            </div>
            <div className="mini-calendar">
              {weekdays.map((weekday, index) => <span className="weekday" key={`${weekday}-${index}`}>{weekday}</span>)}
              {calendarDays.map((day, index) => {
                const currentDate = day ? dateKey(new Date(now.getFullYear(), now.getMonth(), day)) : ''
                const classNames = [day === now.getDate() ? 'today' : '', currentDate && taskDates.has(currentDate) ? 'has-task' : ''].filter(Boolean).join(' ')
          ...
          >
            <div className="card-head">
              <div><p className="eyebrow">{copy.todo}</p><h2>{copy.upcoming}</h2></div>
              <span className="card-view-label">{copy.view} →</span>
            </div>
            <div className="todo-buckets">
              {counts.map((count, index) => <div className="todo-bucket" key={copy.periods[index]}><strong>{count}</strong><span>{copy.periods[index]}</span></div>)}
            </div>
