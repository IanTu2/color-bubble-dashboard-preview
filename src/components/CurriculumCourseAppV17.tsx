...
type Page =
  ...
  | { id: string; kind: 'concept'; title: string; explanation: string; example?: string; misconception?: TextbookUnitContentV14['misconceptions'][number]; index: number; quickCheck?: ReviewedQuestion }
...
const REPORT_OPTIONS: Array<{ id: ReportKind; label: string }> = [
  ...
  { id: 'answer-question', label: '題目／解答有疑問' },
...
function questionGroups(questions: ReviewedQuestion[]) {
  const candidates = questions.filter((question) => !question.id.includes('-ped-v17-check-'))
  if (!candidates.length) return { guided: [], practice: [], assessment: [] }
  const guidedEnd = Math.max(4, Math.floor(candidates.length * 0.34))
  const practiceEnd = Math.max(guidedEnd + 4, Math.floor(candidates.length * 0.67))
  return {
    guided: candidates.slice(0, guidedEnd),
    practice: candidates.slice(guidedEnd, practiceEnd),
    assessment: candidates.slice(practiceEnd),
  }
}
...
function buildPages(unitContent: TextbookUnitContentV14, lesson: CurriculumLessonPlan): Page[] {
  ...
  if (lesson.kind === 'concept') return [
    ...
    ...unitContent.concepts.map((concept, index): Page => ({
      ...
      quickCheck: conceptChecks[index],
      index,
    })),
    { id: `${lesson.id}-recap`, kind: 'recap', title: '把剛才的觀念串起來', items: unitContent.takeaway },
  ]
  ...
  if (lesson.kind === 'example') return [
    ...
    { id: `${lesson.id}-misconception`, kind: 'visual', title: '例題做完，再看最容易犯的錯', visual: misconceptionVisual },
    { id: `${lesson.id}-recap`, kind: 'recap', title: '例題學完了', items: ['先讀完整情境與限制', '選擇合適的表示、模型或證據', '把中間推理寫完整', '主動檢查答案或結論', '換一個情境仍然能重新做一次'] },
  ]
  ...
  const renderPage = () => {
    ...
    return (
      <article className="curriculum-v14-card recap">
        <span className="curriculum-v14-eyebrow">重點整理</span><h2>{page.title}</h2><div className="curriculum-v14-takeaway">{page.items.map((item) => <div key={item}>✓ {item}</div>)}</div>{lesson.kind === 'assessment' ? <div className="curriculum-v17-completion"><strong>{completed.includes(lesson.id) ? '✓ 這次檢核已完成' : '完成後，回頭看哪些地方最需要再練一次'}</strong><p>答對不是唯一目標；能指出自己錯在哪個條件、證據或推理步驟，才代表這一輪學習真的有留下來。</p><button className="curriculum-v14-check" type="button" onClick={markComplete}>{completed.includes(lesson.id) ? '✓ 已記錄完成' : '完成這次檢核'}</button></div> : null}
      </article>
    )
  }
